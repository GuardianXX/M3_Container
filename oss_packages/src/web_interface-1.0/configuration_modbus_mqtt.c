...
void *web_s_configuration(void) {
  ...
  fprintf(output, "<tr><td>%s</td><td>", settings[SETTING_MOSQUITTO_USER].name);
  print_input("text", settings[SETTING_MOSQUITTO_USER].input_name, settings[SETTING_MOSQUITTO_USER].value, "float: right; margin-right: 500px;", "");
  fprintf(output, "</td></tr>");

  fprintf(output, "<tr><td>%s</td><td>", settings[SETTING_MOSQUITTO_PASSWORD].name);
  print_input("text", settings[SETTING_MOSQUITTO_PASSWORD].input_name, settings[SETTING_MOSQUITTO_PASSWORD].value, "float: right; margin-right: 500px;", "");
  fprintf(output, "</td></tr>");
...
void *web_c_configuration(void) {
  ...
  fputs(line, config);
  get_line(line, "mosquitto_user");
  fputs(line, config);
  get_line(line, "mosquitto_password");
  fputs(line, config);
