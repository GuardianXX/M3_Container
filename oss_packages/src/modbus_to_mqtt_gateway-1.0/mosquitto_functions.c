...
struct mosquitto *mosquitto_initialize(char *path_to_config)
{
    ...
    if(getStringFromFile_n(path_to_config, "mosquitto_user", user, DATA_SIZE) == -1) {
        log_entry(APP_NAME, "Error reading broker user");
        printf("Error reading broker user..\n");
        return NULL;
    }

    if(getStringFromFile_n(path_to_config, "mosquitto_password", password, DATA_SIZE) == -1) {
        log_entry(APP_NAME, "Error reading broker password");
        printf("Error reading broker password..\n");
        return NULL;
    }
    ...
    if(mosquitto_username_pw_set(mosq, user, password)) {
        log_entry(APP_NAME, "Error entering credentials to mosquitto");
		fprintf(stderr, "Error entering credentials to mosquitto.\n");
        return NULL;
    }
