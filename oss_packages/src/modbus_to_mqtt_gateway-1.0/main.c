...
/* ######################### Initialize variables #########################  */
    ...
    if(getStringFromFile_n(CONFIG_FILE_PATH, "mosquitto_user", user, DATA_SIZE) == -1) {
        log_entry(APP_NAME, "Error reading user");
        printf("Error reading user..\n");
    ...
    if(getStringFromFile_n(CONFIG_FILE_PATH, "mosquitto_password", password, DATA_SIZE) == -1) {
        log_entry(APP_NAME, "Error reading password");
...
/* ######################### Start application #########################  */
    ...
    while(1)
    {
        ...
        if(diff(&last_read_time, &now) >= pollingInterval * 1000) {
            ...
            char FECHA[100];
            t=time(NULL);
            strftime(FECHA, 100, "%d/%m/%Y  %H:%M:%S", localtime(&t));
            freopen ("Reporte.txt","a",stdout);
            printf (FECHA);
            printf (text);
            fclose (stdout);
            /*ofstream archivo;
